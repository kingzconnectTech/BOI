...
def get_status(bot_state: BotState = Depends(get_current_bot)):
    # Safely get balance using DataFeed's resilient method
    balance = bot_state.data_feed.get_balance()
    
    # If balance is 0 and we think we are connected, it might be a real 0 balance 
    # OR a connection failure that get_balance() couldn't recover from (which sets is_connected=False).
    # So we trust bot_state.data_feed.is_connected updated by get_balance().
